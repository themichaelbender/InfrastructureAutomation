...
#endregion Ch9
#region Ch10 - Error Handling
#region Error Action
$ErrorActionPreference

help about_common_parameters

$ErrorActionPreference = 'continue' #Keep going with Error Displayed
Get-WmiObject -Class Win32_BIOS -ComputerName NotOnline,localhost

$ErrorActionPreference = 'SilentlyContinue' #Keep going without diplaying error
Get-WmiObject -Class Win32_BIOS -ComputerName NotOnline,localhost

$ErrorActionPreference = 'Stop' #Display error and Quit
Get-WmiObject -Class Win32_BIOS -ComputerName NotOnline,localhost

$ErrorActionPreference = 'Inquire' #Prompt for response
Get-WmiObject -Class Win32_BIOS -ComputerName NotOnline,localhost
    ...
    PROCESS {
        ...
        foreach ($computer in $computername) {
            ...
            Try { 
                ...
                #switch parameter $logerrors = $true, than perform catch
            } Catch {
                if ($LogErrors) {
...
Get-SystemInfo –computername NOTONLINE #No error log created, Still produces errors since code runs
...
#>
    ...
    PROCESS {
        ...
        foreach ($computer in $computername) {
            ...
            if ( $everything_ok ) {   #executes remaining code if error ok; skips if not ; Implied true
                $comp = Get-WmiObject -class Win32_ComputerSystem `
                                      -computerName $computer
                $bios = Get-WmiObject -class Win32_BIOS `
                                      -computerName $computer
...
Get-SystemInfo –computername NOTONLINE,ServerA,NOTONLINE2,ServerA,NOTONLINE3,ServerA -verbose
Get-SystemInfo –computername NOTONLINE,ServerA,NOTONLINE2,ServerA,NOTONLINE3,ServerA -logerrors -verbose -ErrorLog C:\scripts\Week13\retry.txt

notepad C:\scripts\Week13\retry.txt
#endregion
...
Get-SystemInfo -ComputerName NOTONLINE,ServerA,NOTONLINE2,ServerA,NOTONLINE3,ServerA -logerrors -ErrorLog C:\scripts\Week13\retry.txt
#endregion
...
#region Chapter 11 - Debugging
    ...
    foreach ($line in $data) {
        #If use to perform specific task if condition met
        #Might need elseif for property value beyond buy & sell
        if ($line.transaction -eq 'buy') {
            # buy transaction (we sold)
            $totalqty -= $line.qty
            $totalsold = $line.total
    ...
    #region 11.6
        ...
        .\Ch11-Debug\listing11-7.ps1 -debug
